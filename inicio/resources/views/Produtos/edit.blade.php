...
<form action="{{ action('ProdutosController@update', $id) }}" method="post" enctype="multipart/form-data">
    ...
    <div class="input-group mb-3">
        ...
        @if (file_exists('./img/produtos/' . md5($produto->id) . '.jpg'))
            <div class="col-md-6">
                <img style="max-width: 200px;" src="{{ url('img/produtos/' . md5($produto->id) . '.jpg') }}"
                    alt="Imagem Produto" class="img-fluid img-thumbnail" id="preview">
            </div>
            @else
...
<div class="modal-content" style="background-color: #456; color: #fff;">
    <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Editar um Produto - {{ $produto->titulo }}</h5>
    ...
    <div class="modal-body">
        ...
        <form action="{{ action('ProdutosController@update', $id) }}" method="post" enctype="multipart/form-data"
            ...
            <div class="input-group mb-3">
                ...
                <input type="number" step=".01" class="form-control" name="preco" id="preco"
                    placeholder="Infome o Valor" value="{{ $produto->preco }}">
            </div>
            <div class="input-group mb-3">
                <label for="imgProduto">Imagem do Produto</label>
                @if (file_exists('./img/produtos/' . md5($produto->id) . '.jpg'))
            ...
            <div class="modal-footer">
                <button type="submit" class="btn btn-success">Editar Produtos</button>
            </div>
        </form>
    </div>
